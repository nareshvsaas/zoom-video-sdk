...
export class CallComponent {
  ...
  constructor(private zoomService: ZoomSdkService, @Inject(PLATFORM_ID) private platformId: Object, private route: ActivatedRoute) {
  ...
  updateUserVideos() {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < this.userVideos.length; i++) {
          const userId = this.userVideos[i];
              ...
              attachPromises.push(
                  mediaStream.attachVideo(userId, 3).then(() => {
                    ...
                    if (user&&user.bVideoOn) {
                      ...
                      console.error(`No video on for user ${userId}`);
                      const videoElement = document.getElementById(userId);
                      if(videoElement)
                      videoElement.style.backgroundColor="#80808096";
  ...
  setupEventListeners(): void {
    this.client.on('user-added', (payload: any) => {
      ...
      this.updateUserVideos();
      this.users = this.userVideos = this.client.getAllUser().map((user: { userId: any; }) => user.userId);
    });
